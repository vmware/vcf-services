...
Metadata is available in the `/cloudapi` REST api.
...
The access control model is based on `domain`. `File` entries have an additional layer of authorization through a right (`Metadata File Entry: Create/Modify`) which enables only specific users to work with this kind of resource(create/update/delete).
